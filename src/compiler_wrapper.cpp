...
bool compiler_wrapper_t::handle_command(const string_list_t& args, int& return_code) {
  ...
  try {
    ...
    hasher.update(get_program_id(args));
...
std::string compiler_wrapper_t::preprocess_source(const string_list_t&) {
  // Default: There is no prepocessing step.
  return std::string();
}

string_list_t compiler_wrapper_t::filter_arguments(const string_list_t& args) {
  // Default: All arguments are relevant.
  return args;
}

std::string compiler_wrapper_t::get_program_id(const string_list_t& args) {
  // Default: The hash of the program binary serves as the program identification.
  const auto& program_exe = args[0];
  hasher_t hasher;
  hasher.update_from_file(program_exe);
  return hasher.final().as_string();
}

std::map<std::string, std::string> compiler_wrapper_t::get_build_files(const string_list_t&) {
